...
## Appendix A.1 — same-day refresh (2026-08-04)

- Opus 5 released Jul 24: $5/$25 API (unchanged from 4.8), default on Max +
  Claude Code, SWE-bench Verified 96 (Anthropic) / 97 (Vals.ai) — quote both.
- Kimi K3: 93.4 on Vals.ai; llm-stats cheap cluster still ~80. Harness spread
  is now a first-class talking point (slide 5 updated).
- GLM Coding Plan mid-transition to credits (Jul 30, docs.z.ai primary) —
  don't quote GLM prices tonight. Kimi Code signups reopened, tiers from $19
  (secondary-sourced). OpenCode Go now 18 models. MiniMax plan pricing
  conflicts across sources — skip.
- Claude/ChatGPT plan prices unchanged. Codex page's "2x through May 31"
  promo text is stale cache — don't cite.
- Slides synced to current OMO Slim v2 presets (Sol/Opus-4.8 openai preset,
  MiniMax-M3-thinking go preset) and Systematic config (gpt-5.5 low review,
  no temperature pin). AA chart replaced with Aug 4 screengrab incl. Claude
  models + Pareto line; click-to-zoom added.
